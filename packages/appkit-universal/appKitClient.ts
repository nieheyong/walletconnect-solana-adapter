...
export type Web3ModalClientOptions = Omit<
  LibraryOptions,
...
export class WalletConnectModal extends Web3ModalScaffold {
  ...
  public constructor(options: Web3ModalClientOptions) {
    ...
    const networkControllerClient: NetworkControllerClient = {
      ...
      getApprovedCaipNetworksData: () => {
        //@ts-ignore
        const accounts =
          this.universalProvider?.session?.namespaces[this.requestedScope]
            .accounts
        const chains = accounts?.map(
          (account) => `${this.requestedScope}:` + account.split(':')[1],
        )
  ...
  private async syncAccount() {
    ...
    if (session) {
      ...
      const chainId =
        session.namespaces?.[connectedScope]?.accounts[0].split(':')[1]
      this.setIsConnected(true)
      this.setCaipAddress(
        session.namespaces?.[connectedScope]
          ?.accounts[0] as `${string}:${string}:${string}`,
      )
      this.setCaipNetwork({
